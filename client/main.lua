local CurrentActionData, handcuffTimer, dragStatus, blipsCopsSheriff, currentTask = {}, {}, {}, {}, {}
...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{label = _U('sheriff_wear'), uniform = grade}
	}
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'sheriff_actions', {
		...
		if data.current.value == 'citizen_interaction' then
			local elements = {
				...
				{label = "Prision Federal",   value = 'jail_menu'}
			}
		...
		elseif data.current.value == 'object_spawner' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				elements = {
					...
					{label = "Barrera rigida", model = 'prop_mp_barrier_02b'},
					{label = "Carpa", model = 'prop_gazebo_02'},
					{label = _U('cash'), model = 'hei_prop_cash_crate_half_full'},
					{label = "Parasol", model = 'prop_parasol_05'},
					{label = "Barrera de cemento", model = 'prop_barier_conc_02a'}
			}}, function(data2, menu2)
				local playerPed = PlayerPedId()
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_sheriffjob:getOtherPlayerData', function(playerData)
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'sheriff' and ESX.PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_sheriffjob:hasEnteredEntityZone', function(entity)
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'sheriff' and IsPedOnFoot(playerPed) then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'sheriff' then
		...
		if IsControlJustReleased(0, 167) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'sheriff' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'sheriff_actions') then
			if not Config.EnableESXService then
...
function createBlip(id)
	...
	if not DoesBlipExist(blip) then -- Add blip and create head display on player
		...
		table.insert(blipsCopsSheriff, blip) -- add blip to array so we can remove it later
	end
end
...
AddEventHandler('esx_sheriffjob:updateBlip', function()
	...
	-- Refresh all blips
	for k, existingBlip in pairs(blipsCopsSheriff) do
	...
	if (ESX.PlayerData.job and ESX.PlayerData.job.name == 'sheriff') or (ESX.PlayerData.job and ESX.PlayerData.job.name == 'police') then
